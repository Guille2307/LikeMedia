...
La ruta `/admin` contiene un panel privado para consultar reservas, mensajes y paquetes publicados. Desde **Paquetes y servicios** puedes crear, editar, archivar/activar y eliminar paquetes; los cambios se guardan en PostgreSQL y el catálogo público solo muestra los activos. El acceso usa correo, contraseña y sesiones JWT de ocho horas. En Railway define `JWT_SECRET` (una cadena aleatoria de al menos 32 caracteres), `ADMIN_EMAIL` (por defecto `info@likemedia.es`), `ADMIN_NAME` y una de estas opciones secretas: `ADMIN_PASSWORD_HASH` (recomendada) o `ADMIN_PASSWORD` (alternativa de migración). No guardes ninguna de ellas en el repositorio. La sesión se conserva únicamente en `sessionStorage` del navegador y se invalida al cerrar sesión o al caducar. `ADMIN_TOKEN` sigue aceptándose temporalmente por la API para no romper clientes antiguos, pero la pantalla ya utiliza JWT. La API aplica además un honeypot y un límite temporal de cinco envíos por email para reducir spam.

## Bloque legal

La web incluye `/legal/aviso-legal`, `/legal/privacidad`, `/legal/cookies` y `/legal/terminos`, además de un banner de consentimiento con aceptar, rechazar y configurar. El responsable indicado es **Dysmar Vidal · 77461330-K**, `info@likemedia.es`, teléfono `+34 624 62 26 94` y domicilio `Rua Manuel Nuñez 4, bajos, oficina B, 36203`. Conviene revisar la situación fiscal, retención y plazos de conservación con un profesional. La política no activa analítica ni publicidad actualmente; cualquier herramienta futura deberá integrarse detrás del consentimiento correspondiente.
